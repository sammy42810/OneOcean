...
This drops and repopulates every collection (beaches, advisories, users, events) with a mix of real and synthetic data:

- **Beaches, advisories, and water-quality scores** are pulled live from California's public ["Beach Water Quality Postings and Closures"](https://data.ca.gov/dataset/beach-water-quality-postings-and-closures) open dataset via its CKAN `datastore_search_sql` API — a curated, deterministic subset of ~30-50 beaches spanning multiple counties and statuses, each beach's water quality derived from its most recent real bacteria monitoring reading. **Requires network access to `data.ca.gov` when you run it.**
- **Demo users, events, comments, and ratings** are synthetic application content generated by the seed script — no public dataset covers those.

> **Note:** `seed.js` calls `dropDatabase()` first, so it wipes the target database — beaches, advisories, users, and events — before reseeding. Point it at a local/dev database only.
...
| Email | Name | Notes |
| --- | --- | --- |
| `demo@oneocean.app` | Demo User | Primary grader login — pre-populated with ratings, comments, bookmarks, and a hosted event |
| `ava.martinez@oneocean.app` | Ava Martinez | |
| `liam.chen@oneocean.app` | Liam Chen | |
| `maya.patel@oneocean.app` | Maya Patel | |
| `noah.garcia@oneocean.app` | Noah Garcia | |
| `zoe.thompson@oneocean.app` | Zoe Thompson | |
| `ethan.brooks@oneocean.app` | Ethan Brooks | |
| `olivia.reyes@oneocean.app` | Olivia Reyes | |

### Relationship to `CS546.OneOceanDB`

The sibling [`CS546.OneOceanDB`](../CS546.OneOceanDB) package defines its own, stricter Mongo `$jsonSchema` validators/indexes over a differently-shaped document contract. `seed.js` does **not** apply those — it writes exclusively through this API's own data layer (`data/*.js`), which validates via `utils/*.js`, and `dropDatabase()` would wipe any validators from `CS546.OneOceanDB`'s `setup` script anyway. Reconciling the two schemas so this seed can also satisfy `CS546.OneOceanDB`'s validators is tracked as a separate, not-yet-started task.
